...
test.describe('sauce automation', () => {
  for (const login of testfile) {
    test(`login with username: ${login.username}`, async ({ page }) => {
      ...
      await test.step('Verify cart button @cart', async () => {
      ...
      await test.step('Verify footer links open correct pages @footer', async () => {
        const footerLinks = [
      ...
      await test.step('Verify product sort dropdown @filter', async () => {
